...
def main():
    ...
    # OVSSwitch protocols='OpenFlow13' forces stable OF1.3 handshakes
    net1 = Mininet(topo=topo, controller=None, switch=lambda name, **kwargs: OVSSwitch(name, protocols='OpenFlow10', **kwargs))
    net1.start()
    ...
    # Forced OpenFlow 1.3 here as well to prevent Floodlight DecoderExceptions
    net2 = Mininet(topo=topo, controller=lambda name: RemoteController(name, ip='127.0.0.1', port=6653), switch=lambda name, **kwargs: OVSSwitch(name, protocols='OpenFlow10', **kwargs))
    net2.start()
