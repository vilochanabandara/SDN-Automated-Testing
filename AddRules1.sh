...
# Clear existing flows from all switches using OpenFlow 1.3
for i in {1..7}; do
    ovs-ofctl -O OpenFlow13 del-flows s$i
    # Allow ARP on all switches so nodes can discover MACs (safely flood in Tree topology)
    ovs-ofctl -O OpenFlow13 add-flow s$i priority=50,arp,actions=flood
done

# Function to add bidirectional flow using OpenFlow 1.3
function route_bidir {
    sw=$1; ip1=$2; p1=$3; ip2=$4; p2=$5
    ovs-ofctl -O OpenFlow13 add-flow $sw priority=100,ip,nw_src=$ip1,nw_dst=$ip2,actions=output:$p2
    ovs-ofctl -O OpenFlow13 add-flow $sw priority=100,ip,nw_src=$ip2,nw_dst=$ip1,actions=output:$p1
}
...
for i in {1..7}; do
    ovs-ofctl -O OpenFlow13 add-flow s$i priority=10,ip,actions=drop
done
